...
void UErosionComponent::InitParameters()
{
	ErosionCycles = 20000;
	Inertia = 0.4; // pInertia
	Capacity = 8; // pCapacity
	MinimalSlope = 1; // pMinSlope
	DepositionSpeed = 0.7; // pDeposition
	ErosionSpeed = 0.6; // pErosion
	Gravity = 0.8; // pGravity
	Evaporation = 0.01; // pEvaporation
	MaxPath = 64; // pMaxPath
	ErosionRadius = 2; // pRadius

	//int32 SampledPointsInRadius = (2 * ErosionRadius + 1) * (2 * ErosionRadius + 1);
	Points.Reserve((2 * ErosionRadius + 1) * (2 * ErosionRadius + 1));
	Weights.Reserve((2 * ErosionRadius + 1) * (2 * ErosionRadius + 1));
}
...
void UErosionComponent::TickComponent(float DeltaTime, ELevelTick TickType,
...
void UErosionComponent::SetHeights(const TArray<float> NewHeights)
{
	...
	{
		GridHeights.Add(NewHeights[Index]);
	}
}

// Useless.
TArray<float> UErosionComponent::GetHeights() const
{
	return GridHeights;
}
...
FDrop UErosionComponent::SetDropInitialParams(const int32 GridSize, const FVector2D Position, const FVector2D Direction,
                                              const float Velocity, const float Water) const
{
	FDrop Drop = FDrop();
	if (Position.X < 0 || Position.X >= GridSize || Position.Y < 0 || Position.Y >= GridSize)
	{
		UE_LOG(LogTemp, Error,
		       TEXT("You inserted an invalid position value! The drop's params will be generated automatically!"));
	...
	{
		UE_LOG(LogTemp, Error,
		       TEXT("You inserted an invalid direction value! The drop's params will be generated automatically!"));
		return GenerateDropInitialParams(GridSize);
	}
...
FVector2D UErosionComponent::GetPairedLinearInterpolation(const FVector2D& OffsetPosition, const float& F1,
                                                          const float& F2, const float& F3, const float& F4) const
{
	// (x, y) = (u, v)
	return FVector2D(
...
FPositionHeights UErosionComponent::GetPositionHeights(const FVector2D& IntegerPosition, const int32 GridSize) const
{
	...
	case No_Error:
		...
		PositionHeights.X_Y1 = GridHeights[(IntegerPosition.X + GridSize) + IntegerPosition.Y * GridSize];
	// P(x, y + 1)
		PositionHeights.X1_Y1 = GridHeights[(IntegerPosition.X + GridSize + 1) + IntegerPosition.Y * GridSize];
	// P(x + 1, y + 1)
		break;
	}
...
float UErosionComponent::GetBilinearInterpolation(const FVector2D& OffsetPosition,
                                                  const FPositionHeights& PositionHeights) const
{
...
void UErosionComponent::SetPointsPositionInRadius(const FVector2D& DropPosition,
...
#pragma endregion
	...
	for (int32 Y = -ErosionRadius; Y <= ErosionRadius; Y++)
	{
		for (int32 X = -ErosionRadius; X <= ErosionRadius; X++)
		{
			...
			Points.Add(FVector2D(PosX, PosY));
		}
	}
}
...
void UErosionComponent::ComputeDepositOnPoints(const FVector2D& IntegerPosition, const FVector2D& OffsetPosition,
                                               const float& Deposit, const int32 GridSize)
{
	switch (GetOutOfBoundResult(IntegerPosition, GridSize))
	{
	case Error_Right_Down:
		...
		break;
	case Error_Right:
		GridHeights[IntegerPosition.X + IntegerPosition.Y * GridSize] += Deposit * (1 - OffsetPosition.X) * (1 -
			OffsetPosition.Y);
		GridHeights[IntegerPosition.X + IntegerPosition.Y * GridSize] += Deposit * OffsetPosition.X * (1 -
			OffsetPosition.Y);
		GridHeights[(IntegerPosition.X + GridSize) + IntegerPosition.Y * GridSize] += Deposit * (1 - OffsetPosition.X) *
			OffsetPosition.Y;
		GridHeights[(IntegerPosition.X + GridSize) + IntegerPosition.Y * GridSize] += Deposit * OffsetPosition.X *
			OffsetPosition.Y;
		break;
	case Error_Down:
		GridHeights[IntegerPosition.X + IntegerPosition.Y * GridSize] += Deposit * (1 - OffsetPosition.X) * (1 -
		...
		GridHeights[IntegerPosition.X + IntegerPosition.Y * GridSize] += Deposit * OffsetPosition.X * (1 -
			OffsetPosition.Y);
		GridHeights[(IntegerPosition.X + 1) + IntegerPosition.Y * GridSize] += Deposit * (1 - OffsetPosition.X) *
			OffsetPosition.Y;
		GridHeights[(IntegerPosition.X + 1) + IntegerPosition.Y * GridSize] += Deposit * OffsetPosition.X *
			OffsetPosition.Y;
		break;
	case No_Error:
		GridHeights[IntegerPosition.X + IntegerPosition.Y * GridSize] += Deposit * (1 - OffsetPosition.X) * (1 -
			OffsetPosition.Y); // P(x, y)
		GridHeights[(IntegerPosition.X + 1) + IntegerPosition.Y * GridSize] += Deposit * OffsetPosition.X * (1 -
			OffsetPosition.Y); // P(x + 1, y)
		GridHeights[(IntegerPosition.X + GridSize) + IntegerPosition.Y * GridSize] += Deposit * (1 - OffsetPosition.X) *
			OffsetPosition.Y; // P(x, y + 1)
		GridHeights[(IntegerPosition.X + GridSize + 1) + IntegerPosition.Y * GridSize] += Deposit * OffsetPosition.X *
			OffsetPosition.Y; // P(x + 1, y + 1)
		break;
	}
}
...
void UErosionComponent::Erosion(FDrop Drop, const int32 GridSize)
{
#pragma region Test
	/*
...
#pragma endregion
		...
		UE_LOG(LogTemp, Warning, TEXT("Gradient on Drop's position: (%f, %f)"), DropPositionGradient.X,
		       DropPositionGradient.Y);
...
#pragma endregion
	...
	UE_LOG(LogTemp, Warning, TEXT("Pointer: %p"), &Points);

	UE_LOG(LogTemp, Warning, TEXT("TESTO A CASO PRIMA"));
	SetPointsPositionInRadius(DropPosition, GridSize);
	UE_LOG(LogTemp, Warning, TEXT("TESTO A CASO DOPO"));
