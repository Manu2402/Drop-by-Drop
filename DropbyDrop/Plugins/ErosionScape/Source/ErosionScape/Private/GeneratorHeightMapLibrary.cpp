...
void UGeneratorHeightMapLibrary::GenerateErosion(TObjectPtr<ALandscape> ActiveLandscape, FErosionSettings& ErosionSettings)
{
...
bool UGeneratorHeightMapLibrary::SaveErosionTemplates(UDataTable* ErosionTemplatesDT)
{
...
void UGeneratorHeightMapLibrary::LoadHeightmapFromFileSystem(const FString& FilePath, TArray<uint16>& OutHeightMap, TArray<float>& OutNormalizedHeightmap, FExternalHeightMapSettings& Settings)
{
	OutHeightMap.Empty();
...
void UGeneratorHeightMapLibrary::CompareHeightmaps(const FString& RawFilePath, const TArray<uint16>& GeneratedHeightmap, int32 Width, int32 Height)
{
...
bool UGeneratorHeightMapLibrary::InitLandscape(TArray<uint16>& HeightData, FHeightMapGenerationSettings& HeightmapSettings, FExternalHeightMapSettings& ExternalSettings, FLandscapeGenerationSettings& LandscapeSettings)
{
...
void UGeneratorHeightMapLibrary::CreateLandscapeFromInternalHeightMap(FHeightMapGenerationSettings& HeightmapSettings, FExternalHeightMapSettings& ExternalSettings, FLandscapeGenerationSettings& LandscapeSettings)
{
...
bool UGeneratorHeightMapLibrary::SetLandscapeSizeParam(int32& SubSectionSizeQuads, int32& NumSubsections, int32& MaxX, int32& MaxY, const int32 Size)
{
...
TArray<uint16> UGeneratorHeightMapLibrary::ConvertArrayFromFloatToUInt16(const TArray<float>& FloatData)
{
...
FVector UGeneratorHeightMapLibrary::GetWindPreviewStart(const ALandscape* ActiveLandscape, UWorld* World)
{
...
float UGeneratorHeightMapLibrary::GetWindPreviewScale()
{
	return FDropByDropSettings::Get().GetWindPreviewScale();
}

void UGeneratorHeightMapLibrary::SetWindPreviewScale(const float NewScale)
{
	FDropByDropSettings::Get().SetWindPreviewScale(NewScale);
}

void UGeneratorHeightMapLibrary::DrawWindDirectionPreview(const FErosionSettings& ErosionSettings, const ALandscape* ActiveLandscape, float ArrowLength, float ArrowThickness, float ArrowHeadSize, float Duration, bool  bAlsoDrawCone, float ConeHalfAngleDeg)
{
#if WITH_EDITOR
